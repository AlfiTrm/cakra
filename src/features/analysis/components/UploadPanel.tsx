...
export function UploadPanel({ file, onFileChange }: UploadPanelProps) {
  ...
  return (
    <section className="rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-white p-5 shadow-lg shadow-[rgb(15_23_42_/_0.06)] sm:p-6">
      <button
        className={`flex min-h-[176px] w-full flex-col items-center justify-center rounded-[var(--radius-xl)] border-2 border-dashed px-6 py-10 text-center transition-colors ${
          inlineError
      ...
      >
        ...
        >
          {inlineError ? <AlertIcon /> : isDragging ? null : <UploadIcon />}
        </span>
        <span
        ...
        >
          {inlineError ? 'Format file tidak didukung' : isDragging ? 'Lepaskan file untuk mengunggah' : 'Seret file CSV/XLSX ke sini atau klik untuk memilih'}
        </span>
      ...
      {file && !inlineError ? <UploadedFileCard file={file} /> : null}
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          <DownloadIcon />
          Unduh template CSV/XLSX contoh
        </a>
        {inlineError ? (
          <button
...
function UploadedFileCard({ file }: { file: UploadFileState }) {
  return (
    <div className="mt-5 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-neutral-50)] p-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <p className="flex items-center gap-2 text-label-sm font-bold text-[var(--color-success)]">
          <CheckCircleIcon />
          File valid
        </p>
      </div>
      <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-body-sm text-[var(--color-text-muted)]">
        ...
        <ValidationItem label={`${file.rows} baris terdeteksi`} />
      </div>
    </div>
  )
}
